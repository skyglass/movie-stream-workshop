...
stored as the winner over the other movie in `user_movie_winner_loser`, and the full transitive closure is stored in
...
Each write updates direct and transitive rows atomically in the Movie Challenge transaction. Cycles are rejected by
checking whether the reverse winner-loser relationship already exists in the closure table. Favorite and recommendation
views calculate vote strength from the closure table by counting rows where a movie is the winner.
...
Feature: movie-challenge
  ...
  Scenario: Movie Challenge winner updates direct and transitive winner-loser tables
    Given movie "tt101" exists with title "Movie One"
    And movie "tt102" exists with title "Movie Two"
    And movie "tt101" is already recommended by "user"
    And movie "tt102" is already recommended by "user"
