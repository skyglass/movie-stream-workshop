...
public class ViewFavoriteMoviesUseCase {
    ...
    @Transactional
    public List<MovieDto> viewFavoriteMovies(Jwt jwt) {
        UserExtra userExtra = userExtraService.syncFromJwt(jwt);
        return viewFavoriteMovies(userExtra.getUsername());
    }

    @Transactional(readOnly = true)
    public List<MovieDto> viewFavoriteMovies(String username) {
        Set<String> recommendedMovieIds = movieRecommendationService.recommendedMovieIds(username);
        return movieService.getFavoriteMovies(username).stream()
