...
import { EditFavoriteRanksDialogComponent } from './edit-favorite-ranks-dialog';

describe('EditFavoriteRanksDialogComponent', () => {
  let fixture: ComponentFixture<EditFavoriteRanksDialogComponent>;
  let component: EditFavoriteRanksDialogComponent;
  let api: jasmine.SpyObj<MoviesApiService>;

  beforeEach(() => {
    api = jasmine.createSpyObj<MoviesApiService>('MoviesApiService',
      ['listFavoriteMovies', 'submitFavoriteMoviesRanking']);
    api.listFavoriteMovies.and.returnValues(
      of({ movies: movies(1, 100), totalCount: 250 }),
      of({ movies: movies(101, 100), totalCount: 250 }));

    TestBed.configureTestingModule({
      imports: [EditFavoriteRanksDialogComponent],
      providers: [{ provide: MoviesApiService, useValue: api }]
    });
    fixture = TestBed.createComponent(EditFavoriteRanksDialogComponent);
  ...
  it('appends the next 100 without changing any edited rank in the existing prefix', () => {
    ...
    expect(api.listFavoriteMovies.calls.argsFor(1)).toEqual([2, 100]);
  });
  ...
  it('closes without submitting anything', () => {
    ...
    expect(api.submitFavoriteMoviesRanking).not.toHaveBeenCalled();
  });
});
