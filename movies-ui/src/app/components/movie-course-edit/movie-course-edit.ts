...
export class MovieCourseEditComponent implements OnInit {
  ...
  editingMovie:CourseMovie|null=null;
  ...
  saveMovie():void{
    if(!this.course||!this.editingMovie)return;
    const movie=this.editingMovie;
    this.busy=true;
    this.api.updateCourseMovie(this.course.id,movie.imdbId,{movieId:movie.imdbId,header:this.headers[movie.imdbId],description:this.descriptions[movie.imdbId],watchOrder:movie.watchOrder,linkedCourseId:movie.linkedCourseId}).subscribe({
      next:c=>{this.setCourse(c);this.editingMovie=null;this.busy=false;},
      error:e=>this.fail(e)
    });
  }

  confirmRemove(movie:CourseMovie):void{this.confirmRemoveMovie=movie;}
  cancelRemove():void{this.confirmRemoveMovie=null;}
  removeConfirmed():void{
    if(!this.course||!this.confirmRemoveMovie)return;
    const movie=this.confirmRemoveMovie;
    this.busy=true;
    this.api.removeCourseMovie(this.course.id,movie.imdbId).subscribe({
      next:c=>{this.setCourse(c);this.confirmRemoveMovie=null;this.busy=false;},
      error:e=>{this.confirmRemoveMovie=null;this.fail(e);}
    });
  }

  requestDeleteCourse():void{this.confirmDeleteCourse=true;}
  cancelDeleteCourse():void{this.confirmDeleteCourse=false;}
  deleteCourseConfirmed():void{
    if(!this.course)return;
    this.api.deleteMovieCourse(this.course.id).subscribe({next:()=>this.router.navigate(['/movie-journeys']),error:e=>this.fail(e)});
  }
