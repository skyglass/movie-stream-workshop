...
export class MovieCourseEditComponent implements OnInit, OnDestroy {
  ...
  course?:MovieCourse;header='';title='';description='';filter='';results:Movie[]=[];loading=true;busy=false;errorMessage='';addingMovie=false;
  ...
  saveCourse():void{if(!this.course)return;this.busy=true;this.api.updateMovieCourse(this.course.id,this.header,this.title,this.description).subscribe({next:c=>this.router.navigate(['/movie-journeys',c.id]),error:e=>this.fail(e)});}
  ...
  private setCourse(c:MovieCourse):void{this.course=c;this.header=c.header;this.title=c.title;this.description=c.description;c.movies.forEach(m=>{this.headers[m.imdbId]=m.header;this.descriptions[m.imdbId]=m.description;});}
