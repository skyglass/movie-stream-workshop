...
export class MovieCourseEditComponent implements OnInit {
  ...
  course?:MovieCourse;allCourses:MovieCourse[]=[];title='';description='';filter='';results:Movie[]=[];loading=true;busy=false;errorMessage='';
  ...
  saveCourse():void{if(!this.course)return;this.busy=true;this.api.updateMovieCourse(this.course.id,this.title,this.description).subscribe({next:c=>{this.setCourse(c);this.busy=false;},error:e=>this.fail(e)});}
  ...
  private setCourse(c:MovieCourse):void{this.course=c;this.title=c.title;this.description=c.description;c.movies.forEach(m=>{this.descriptions[m.imdbId]=m.description;this.orders[m.imdbId]=m.watchOrder;this.links[m.imdbId]=m.linkedCourseId;});}
